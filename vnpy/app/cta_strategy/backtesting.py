...
class OptimizationSetting:
    ...
    def add_parameter(
        ...
        if start >= end:
            ...
            print("Parameter optimization start point must be less than end point")
            return
...
class BacktestingEngine:
    ...
    def load_data(self):
        ...
        if self.start >= self.end:
            ...
            self.output("Start date must be less than end date")
            return
        ...
        self.output(f"Loading of historical data is complete, the amount of data：{len(self.history_data)}")
    ...
    def run_backtesting(self):
        ...
        self.output("Policy initialization completed")
    ...
    def calculate_result(self):
        ...
        self.output("Start calculating daily mark-to-market profit and loss")

        if not self.trades:
            # BRIAN: 成交记录为空，无法计算 - 
            # Transaction record is empty and cannot be calculated
            self.output("Transaction record is empty and cannot be calculated")
            return
        ...
        self.output("Completion of daily mark-to-market profit and loss calculation")
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True):
        ...
        if output:
            ...
            self.output(f"Lost trading days:\t{loss_days}")
            ...
            self.output(f"Total transactions:\t{total_trade_count}")

            # BRIAN: 日均盈亏 - Daily avg. profit and loss
            self.output(f"Daily avg. profit and loss:\t{daily_net_pnl:,.2f}")
            
            # BRIAN: 日均手续费 - Daily avg. commission
            self.output(f" Daily avg. commission:\t{daily_commission:,.2f}")
            # BRIAN: 日均滑点 - Daily slippage
            self.output(f"Daily slippage:\t{daily_slippage:,.2f}")
            # BRIAN: 日均成交金额 - Daily avg. turnover
            self.output(f"Daily avg. turnover:\t{daily_turnover:,.2f}")
            # BRIAN: 日均成交笔数 - Daily avg. transactions
            self.output(f"Daily avg. transactions:\t{daily_trade_count}")

            # BRIAN: 日均收益率 - Daily avg. return
            self.output(f"Daily avg. return:\t{daily_return:,.2f}%")
            self.output(f"Std of returns:\t{return_std:,.2f}%")
        ...
        self.output("Computation of strategy statistics indicators")
        return statistics
    ...
    def run_optimization(self, optimization_setting: OptimizationSetting, output=True):
        ...
        if not settings:
            ...
            self.output("Optimization parameter combination is empty, please check")
            return
    ...
    def run_ga_optimization(self, optimization_setting: OptimizationSetting, population_size=100, ngen_size=30, output=True):
        ...
        self.output(f"Total population per generation: {pop_size}")
        # BRIAN: 优良筛选个数 - Excellent screening number
        self.output(f"Excellent screening number: {mu}")
        # BRIAN: 迭代次数 - Num. of iterations
        self.output(f"Num. of iterations: {ngen}")
        # BRIAN: 交叉概率 - Cross probability
        self.output(f"Cross probability: {cxpb:.0%}")
        ...
        # Genetic algorithm optimization completed, it took {cost} seconds
        self.output(f"Genetic algorithm optimization completed, it took {cost} seconds")
